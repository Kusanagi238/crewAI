...
def test_csv_knowledge_source_updates():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if not hasattr(csv_source, "files_have_changed"):
            print(
                "❌ TEST FAILED: files_have_changed method not found in CSVKnowledgeSource"
            )
            return False

        if not hasattr(csv_source, "_file_mtimes"):
            print(
                "❌ TEST FAILED: _file_mtimes attribute not found in CSVKnowledgeSource"
            )
            return False
        ...
        if not hasattr(knowledge, "_check_and_reload_sources"):
            print(
                "❌ TEST FAILED: _check_and_reload_sources method not found in Knowledge"
            )
            return False
